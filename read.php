...
try {
  ...
  //orderパラメータの値が存在すれば（並べ替えボタンを押したとき）、その値を変数$orderに代入する
  if (isset($_GET['order'])) {
    $order = $_GET['order'];
  } else {
    $order = NULL;
  ...
  // SQLのLINK句で使うため、変数$keyword（検索ワード）の前後を％で囲む（部分一致）
...
<body>
  ...
  <main>
    <article class="products">
      <h1>商品一覧</h1>
      <?php
      // （商品登録・編集・削除）messageパラメータの値を受け取っていれば、それを表示する
      ...
      <div class="products-ui">
        <div>
          ...
          <form action="read.php" method="get" class="search-form">
            ...
            <input type="text" class="search-box" placeholder="商品目で検索" name="keyword" value="<?= $keyword ?>">
          </form>
        </div>
