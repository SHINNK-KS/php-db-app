...
if (isset($_GET['id'])) {
  try {
    ...
    // idパラメータの値をプレースホルダ（：ID）に置き換えたSQL文をあらかじめ用意する
    ...
    // SQL文の実行結果を配列で取得する
    // 補足：PDO::RETCH_COLUMNは1つのカラムの値を1次元配列（多次元ではない普通の配列）で取得する設定である
        ...
        <div>
          ...
          <select id="vendor_code" name="vendor_code" required>
            ...
            foreach ($vendor_codes as $vendor_code) {
              // もし変数$vendor_codeが商品の仕入先コードの値と一致すれば、selected属性をつけて初期値にする
